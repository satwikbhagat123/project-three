...
function B(_) {
  try {
    ...
    if (t.message.indexOf("is not a constructor") >= 0)
      return false;
  }
  return true;
}
...
var z = class extends Material {
  constructor({
    ...
    const n = i;
    n.name = `CustomShaderMaterial<${i.name || i.type}>`, n.update = this.update.bind(n), n.__csm = { prevOnBeforeCompile: i.onBeforeCompile };
    const r = { ...n.uniforms || {}, ...d || {} };
    n.uniforms = this.uniforms = r, n.vertexShader = this.vertexShader = M || "", n.fragmentShader = this.fragmentShader = u || "", n.update({
  ...
  update({
    ...
    const f = D(M || ""), c = D(t || ""), i = this;
    u && (i.uniforms = u), M && (i.vertexShader = M), t && (i.fragmentShader = t), Object.entries(k).forEach(([a, r]) => {
      for (const l in r) {
        const s = r[l];
        (c && c.includes(s) || f && f.includes(s)) && (i[a] || (i[a] = 1));
      }
    });
    const p = i.__csm.prevOnBeforeCompile, n = (a, r, l) => {
      var A;
      let s, m = "";
      if (r) {
        const o = /void\s+main\s*\(\s*\)[^{]*{((?:[^{}]+|{(?:[^{}]+|{(?:[^{}]+|{(?:[^{}]+|{[^{}]*})*})*})*})*})/gm;
        s = (A = r.matchAll(o).next().value) == null ? void 0 : A[1], s && (s = s.slice(0, -1));
        const C = /void\s+main\s*\(\s*\)\s*{/gm, S = r.search(C);
        m = r.slice(0, S);
      }
      if (l && (r && r.includes(e.fragColor)) && s && (s = `csm_UnlitFac = 1.0;
` + s), a.includes("//~CSM_DEFAULTS")) {
    ...
    i.onBeforeCompile = (a, r) => {
      p == null || p(a, r);
      const l = R || {}, s = i.type, m = s ? `#define IS_${s.toUpperCase()};
` : `#define IS_UNKNOWN;
`;
      a.vertexShader = m + `#define IS_VERTEX
` + a.vertexShader, a.fragmentShader = m + `#define IS_FRAGMENT
` + a.fragmentShader;
      const v = (A) => {
        for (const o in A) {
          ...
          if (o === "*" || c && c.includes(o) || I) {
            ...
            for (const g in $) {
              ...
              } else h && (a.vertexShader = a.vertexShader.replace(
                g,
                h
      ...
      v(w), v(l), a.vertexShader = n(
        ...
        c,
        true
      ), u && (a.uniforms = { ...a.uniforms, ...i.uniforms }), i.uniforms = a.uniforms;
    };
    const E = i.customProgramCacheKey;
    i.customProgramCacheKey = () => ((d == null ? void 0 : d()) || U((f || "") + (c || ""))) + (E == null ? void 0 : E.call(i)), i.needsUpdate = true;
  }
};
export {
  z as default
